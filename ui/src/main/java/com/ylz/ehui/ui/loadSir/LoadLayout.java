...
public class LoadLayout extends FrameLayout {
    ...
    private void postToMainThread(final Class<? extends Callback> status, final String msg) {
        post(new Runnable() {
            @Override
            public void run() {
                showCallbackView(status, msg);
            }
        });
    }

    private void showCallbackView(Class<? extends Callback> status, String msg) {
        if (preCallback != null) {
        ...
        for (Class key : callbacks.keySet()) {
            if (key == status) {
                ...
                if (key == SuccessCallback.class) {
                    ...
                    addView(rootView);
                    callbacks.get(key).onAttach(context, rootView, msg);
                }
                preCallback = status;
            }
        }
